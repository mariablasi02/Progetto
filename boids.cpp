...
Stats statistic(Boids& b, double const delta_t){
  ...
  auto mean_dist2 = (std::inner_product(distances.begin(), distances.end(),
                       ...
                       static_cast<int>(distances.size());
  auto std_dist =
      std::sqrt(mean_dist2 - mean_dist * mean_dist) /
  ...
  auto std_speed =
      std::sqrt(mean_speed2 - mean_speed * mean_speed) /
...
std::string state(Boids& b, double const delta_t) {
 auto data = statistic(b, delta_t);
  ...
  return "Mean position and standard deviation: " + std::to_string(data.mean_distance) + " +/- " + std::to_string(data.std_distance) +
         '\n' + "Mean velocity and standard deviation: " + std::to_string(data.mean_speed) + " +/- " +
